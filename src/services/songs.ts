...
const getCollaboratorAmountToPay = (collaborator: Record<string, unknown>): number => {
  ...
  const splits = collaborator?.splitPayment as
    | Array<{ calculation?: { amountToPay?: number } }>
    | undefined;
...
export const validatePayAllPayment = ({
  ...
  const collaborators = Array.isArray(song?.collaborators)
    ? (song.collaborators as Record<string, unknown>[])
    : [];
  ...
  if (!isStripeConnected) {
    issues.push({
      ...
      severity: "error",
      message:
        "No puedes pagar a todos porque aún no has vinculado y verificado tu cuenta bancaria (ACH) en Billetera.",
    });
  }
