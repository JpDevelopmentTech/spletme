...
const chartButtons: { view: ChartView; icon: React.ReactNode; label: string }[] = [
...
const PlatformsCard = () => {
  ...
  const horizontalSeries = useMemo(() => [{
    name: "Streams",
    data: visibleData.map((p) => ({ x: p.name.split(" ")[0], y: p.streams })),
  }], [visibleData]);
        ...
        {loading ? (
          ...
          <>
            ...
            {chartView === "horizontal" && (
              <div className="h-[160px]">
                <ReactApexChart options={horizontalOptions} series={horizontalSeries} type="bar" height="100%" width="100%" />
              </div>
            )}
