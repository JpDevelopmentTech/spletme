...
function InfoRow({ icon, label, value, accent, action, hasBorder = true }: InfoRowProps) {
  return (
    <div
      ...
      style={{
        height: 60,
        borderBottom: hasBorder ? "1px solid #E5E7EB" : undefined,
      }}
    >
      <div
        ...
        style={{
          ...
          height: 36,
          borderRadius: 8,
          backgroundColor: accent ? "#FDBA74" : "#F3F4F6",
        }}
      >
        {icon}
      </div>
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        ...
        <span
          className={`text-sm font-${accent ? "bold" : "medium"} truncate`}
...
export function ProfileInfoCard({ userData, copied, onCopyId }: ProfileInfoCardProps) {
  return (
    <div className="overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white">
      ...
      />
      <InfoRow
        icon={<AtSign size={16} color="#9CA3AF" />}
