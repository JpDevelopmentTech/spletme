...
export const useSplits = () => {
  ...
  // Actualizar split
  const updateSplit = useCallback(async (splitId: string, data: UpdateSplitRequest): Promise<Split | null> => {
    setLoading(true);
    setError(null);
  ...
  const calculateTotalPercentage = useCallback(async (
    songId: string, 
    ...
    try {
      const result = await splitsService.calculateTotalPercentage(songId, excludeCollaboratorId, additionalPercentage);
      return result;
    } catch (err) {
